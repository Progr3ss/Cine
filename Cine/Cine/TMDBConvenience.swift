...
extension TMDBClient {
	...
	*/
	...
	private func loginWithGuestToken(_ requestToken: String?, hostViewController: UIViewController, completionHandlerForLogin: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
	...
	private func getGuestSessionID(_ requestToken: String?, completionHandlerForSession: @escaping (_ success: Bool, _ sessionID: String?, _ errorString: String?) -> Void) {
		
		/* 1. Specify parameters, method (if has {key}), and HTTP body (if POST) */
		let parameters = [TMDBClient.ParameterKeys.RequestToken: requestToken!]
		
		/* 2. Make the request */
		let _ = taskForGETMethod(Methods.AuthenticationGuestSession , parameters: parameters as [String:AnyObject]) { (results, error) in
			...
			if let error = error {
				...
				if let sessionID = results?[TMDBClient.JSONResponseKeys.GuestSessionID] as? String {
					completionHandlerForSession(true, sessionID, nil)
				} else {
					print("Could not find \(TMDBClient.JSONResponseKeys.GuestSessionID) in \(results)")
					completionHandlerForSession(false, nil, "Login Failed (Session ID).")
				}
			}
		}
